...
export default function Home() {
    ...
    const [sequenceHelper, setSequenceHelper] = useState("Waiting for input...");
    const fileInputRef = useRef<HTMLInputElement>(null);

    const appendSequenceInput = (incoming: string, sourceLabel: string) => {
        const trimmed = incoming.trim();
        if (!trimmed) {
            setSequenceHelper("읽을 수 있는 서열이 없습니다.");
            return;
        }
        setSequenceInput((prev) => (prev ? `${prev.trimEnd()}\n${trimmed}` : trimmed));
        setSequenceHelper(sourceLabel);
    };
    ...
    const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            appendSequenceInput(text, `${file.name}에서 불러왔습니다.`);
        } catch (error) {
            console.error(error);
            setSequenceHelper("파일을 읽을 수 없습니다.");
        } finally {
            event.target.value = "";
        }
    };
    ...
    const handlePasteFromClipboard = async () => {
        try {
            ...
            appendSequenceInput(clipboardText, "클립보드에서 붙여넣었습니다.");
        } catch (error) {
            console.error(error);
            setSequenceHelper("클립보드 접근이 차단되었습니다.");
        }
    };

    const handleCleanSequence = () => {
        setSequenceInput("");
        setSequenceHelper("입력 내용을 모두 지웠습니다.");
    };

    const normalizedSequence = sequenceInput.replace(/^>.*$/gm, "").replace(/[^A-Za-z]/g, "");
    const normalizedSequenceLength = normalizedSequence.length;
                                ...
                                <label className="flex flex-col flex-1 h-full">
                                    <div className="flex justify-between items-end mb-2">
                                        ...
                                        <span className="text-xs text-slate-500 font-mono">
                                            {normalizedSequenceLength.toLocaleString()} bp
                                        </span>
                                    </div>
...
ATGCGT..."
                                        ...
                                        <div className="absolute bottom-4 right-4 flex items-center gap-1.5 text-xs font-bold text-slate-400 bg-slate-800/80 px-2 py-1 rounded border border-slate-700">
                                            ...
                                            {normalizedSequenceLength
                                                ? `${normalizedSequenceLength.toLocaleString()} bp 로드됨`
                                                : sequenceHelper}
                                        </div>
                                    </div>
                                </label>
